...
\[ \text{Unweighted Average Recall(UAR)} =mean(R1, R2, R3,бн, R6)= \frac{0.809 + 0.793 + \dotsm + 0.782}{6} = 0.744 \]
\end{center}
...
\[ \text{Unweighted Average Recall(UAR)} =mean(R1, R2, R3,бн, R6)= \frac{0.545 + 0.679 + \dotsm + 0.705}{6} = 0.613 \]
...
From F1 distributions for that using clean data and noisy data, there are more imbalanced Test Set Conclusions when noisy data is used. This is because when using noisy data to test, the tree would give more result of all zeros, which means none of the emotion. By this case, the algorithm will judge that as Anger, which is the first emotion in the classes. As a result, the F1 for \textit{Anger}\par
